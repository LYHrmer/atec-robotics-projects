...
这个仓库记录三项机器人任务：让 D1+G2 走完整条复杂赛道，让 Piper 机械臂把三个物体放入篮中，以及让 B2wPiper 接近并搬运地面物体。代码、文档和验收证据统一管理，便于复现、学习和回顾工程取舍。
...
| 你想做什么 | Task A | Task B | Task E |
| --- | --- | --- | --- |
| **先看懂机器人在做什么** | [行走与导航导读](task_a/docs/LEARNING_GUIDE.md) | [首分方法与视频](docs/TASK_B_FIRST_SCORE.md) | [从抓取视频开始](docs/LEARNING_GUIDE.md) |
| **跟着代码理解实现** | [代码、模型与运行入口](task_a/README.md) | [控制与运行入口](task_b/README.md) | [一次控制调用的代码导读](docs/CODE_WALKTHROUGH.md) |
...
先按各任务运行文档配置环境与外部资产：[Task A](task_a/README.md) · [Task B](task_b/README.md) · [Task E](docs/TASK_E.md#本地复现)。Task A 的机器人原始资源及基础策略需要按其说明准备；训练后的 [残差模型](task_a/weights/model_1999_final.pt) 已随本仓库保存。
