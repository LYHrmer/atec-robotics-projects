...
arm q, and hold for a short observation. Inputs after entry are the public
...
grasp, transport, delivery or Task B completion. The real check is an external
audit of the same object's recorded pose and mesh lower surface.
"""
...
# ---------------------------------------------------------------------------
# Fixed constants of this FIRST candidate. None of them is tuned on a reward,
# a score, an object pose or any privileged simulator observation.
# ---------------------------------------------------------------------------
CLOSE_PRELOAD_M = .015          # physical q7=-.015 m, q8=+.015 m, as Task E does
...
EMPTY_HOLD_S = .2
PAIRED_DELTA_RAD = .10          # q2 -= .10, q3 += .10; all other angles unchanged
LIFT_MAX_S = 5.
LIFT_RATE_RAD_S = .10
LIFT_TETHER_RAD = .10
LIFT_ERROR_RAD = .04
...
CANDIDATE_CONSTANTS = {
    'candidate': 'first fixed close-and-small-lift candidate; not tuned on score, '
                 'object state or simulator truth',
    'finger_close_target': {'source': 'task_b.arm_kinematics.gripper_targets(0, preload_m=.015)',
                            'physical_q7_m': -CLOSE_PRELOAD_M, 'physical_q8_m': CLOSE_PRELOAD_M,
                            'slew_m_per_s': FINGER_SLEW_M_S,
                            'note': 'a finite target beyond the closed stop requests holding force, '
                                    'exactly as the existing Task E close does; joint limits and '
                                    'actuators are untouched and real motion is limited by physics'},
...
STOP_REASONS = {
    ...
    'grasp_probe_empty_close': 'measured jaw width stayed below .006 m after the closed target was '
                               'commanded',
    'grasp_probe_close_not_confirmed': 'no stable non-empty width window formed inside 3 s',
    'grasp_probe_lift_goal_outside_joint_limits': 'the latched paired goal left the static joint '
                                                  'limits, so nothing was commanded',
    'no_joint_lift_progress': 'a full 2 s active lift window improved the goal error by less than '
                              '.01 rad while it exceeded .04 rad',
    'grasp_probe_lift_not_reached': 'the arrival conditions were not held for .3 s inside 5 s',
    'grasp_probe_empty_during_lift': 'jaw width collapsed below .006 m during the lift',
    'grasp_probe_empty_during_observation': 'jaw width collapsed below .006 m during the observation',
    'grasp_probe_pause_timeout': 'one public body-motion pause reached 2 s',
    'grasp_probe_pause_budget_exceeded': 'cumulative pausing reached 2 s across the probe',
    'grasp_probe_base_displacement_budget_exceeded': 'integrated tangential drift since entry passed .03 m',
    'grasp_probe_base_yaw_budget_exceeded': 'integrated yaw since entry passed .05 rad',
    'grasp_probe_gravity_changed': 'public gravity direction turned more than .05 rad since entry',
    'grasp_probe_total_budget_exceeded': 'the 10 s probe budget, pauses included, ran out',
...
class GraspProbePolicy:
    ...
    def __init__(self, schema, observation_joint_names, defaults, dt=.02, *,
        ...
        # Static limit box for the six arm joints: the shared chain limits
        # intersected with the live soft position limits of this articulation.
        soft = schema.soft_joint_pos_limits[[schema.joint_index(name) for name in ARM6]]
        if not np.isfinite(soft).all():
            raise ValueError('The arm needs finite soft position limits to check a lift goal')
        self.static_lower = np.maximum(JOINT_LOWER, soft[:, 0])
        self.static_upper = np.minimum(JOINT_UPPER, soft[:, 1])
    ...
    def _close(self, q):
        ...
        The stable width window only permits the bounded lift below. It is NOT
        a claim of contact or grasp: no contact sensor, score or object state is
        ...
        if self.close_active_calls*self.dt >= CLOSE_MIN_ACTIVE_S and stable:
    ...
    def _track_lift_goal(self, q):
        ...
        step = np.clip(self.lift_goal-self.arm_command[:6],
                       -LIFT_RATE_RAD_S*self.dt, LIFT_RATE_RAD_S*self.dt)
        tethered = np.clip(self.arm_command[:6]+step, q[:6]-LIFT_TETHER_RAD, q[:6]+LIFT_TETHER_RAD)
        limited = np.clip(tethered, self.static_lower, self.static_upper)
        if float(np.max(np.abs(limited-tethered))) > 1e-12:
            self.lift_clamped_calls += 1  # reported, never silent
        self.arm_command[:6] = limited
    ...
    def describe(self):
        return dict(mode='grasp_probe',
                    ...
                    phases={'PROBE_CLOSE': 'hold the six achieved joint commands, slew both fingers to '
                                           'the fixed physical preload target at <=.05 m/s, and require '
                                           '>=1.4 s of active closing plus ten consecutive width '
                                           'samples with range <.002 m and median in [.006,.065] m. '
                                           'This permits the bounded lift only; it is not a claim of '
                                           'contact or grasp, and there is no re-squeeze or retry',
                            'PROBE_LIFT': 'latch measured arm q once, apply the fixed paired goal '
                                          '(q2-.10 rad, q3+.10 rad, other angles unchanged) after a '
                                          'static limit check, and drive it under a .10 rad/s per-axis '
                                          'rate and a +/-.10 rad measured tether while the closed '
                            ...
                            'PROBE_OBSERVE': 'hold the same measured goal, closed target, lowered leg '
                                             'reference, wheel anchor and motion bounds for 2 s, then '
                                             'end. The experiment stops there: no transport, opening, '
                                             'regrasp or further lowering'},
