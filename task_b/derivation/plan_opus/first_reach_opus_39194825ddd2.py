...
class FirstReachPolicy:
    ...
    def _reach(self, obs, q, sampled, selected):
        ...
        self.debug.update(reach_goal_q=self.reach_q.tolist(),
                          ...
                          reach_subphase='DRIVE_TO_FIXED_GOAL',
                          reach_settle_episodes=self.reach_settle_episodes,
        ...
        if not self.reach_only:
            ...
            if self.lowering_m > 0. and violation:
                return self._finish('reach_base_not_stationary')
            ...
            if sampled:
                if selected is None:
                    ...
                    if displacement > .12:
                        # Only the strict lowering path may call an unmatched
                        # distant component a moved target; in P1 it is simply
                        # not associated with the localized point.
                        if self.lowering_m > 0.:
                            return self._finish('reach_target_moved_beyond_12cm')
            ...
            if self.lowering_m == 0.:
                # The stationary gate already authorized this single fixed goal
                # from two stationary raw detections. Missing candidates, a
                # shape rejection, an unavailable camera or an out-of-frame
                # prediction are diagnostics here: requiring continued shape
                # detections would be an impossible condition while the arm
                # camera sweeps away, and no lowering is authorized.
                ...
                self.state_reason = ('bounded_joint_reach_after_stationary_visual_localization'
                                     if self.reach_visual_valid else
                                     'bounded_joint_reach_visual_association_optional_no_lowering')
            else:
                # Retained strict branch: lowering is not authorized by this
                # task and still requires continuous visual support.
                out_of_view_reach_allowed = expected_out_of_view and self.lowering_m == 0.
                self.debug.update(expected_out_of_view=expected_out_of_view,
                                  out_of_view_reach_allowed=out_of_view_reach_allowed,
                                  reach_visual_valid=self.reach_visual_valid,
                                  reach_valid_samples=self.reach_valid_samples)
                if not self.reach_visual_valid and not out_of_view_reach_allowed:
                    self.state, self.state_reason = 'REACH_VISUAL_HOLD', 'missing_detection_inside_predicted_view_or_lowering_requires_vision'
                    self.reach_lost_start = self.calls if self.reach_lost_start is None else self.reach_lost_start
                    if (self.calls-self.reach_lost_start)*self.dt >= .5:
                        return self._finish('reach_visual_confirmation_lost')
                    return self._action()
                self.reach_lost_start = None
                if out_of_view_reach_allowed and not self.reach_visual_valid:
                    self.state_reason = 'expected_out_of_view_initial_visual_then_joint_feedback_no_lowering'
        if not self.reach_only and self.lowering_m == 0.:
            # Single-factor zero-lowering pause. The drift budgets above and
            # the scheduled detector have already run on this call, and the
            # state stays REACH because the wheel hold depends on it.
            settling = self._reach_settle(q, motion, violation)
            if settling is not None:
                return settling
        age = (self.calls-self.reach_start)*self.dt
        error = float(np.max(np.abs(q[:6]-self.reach_q)))
        self.debug.update(reach_age_s=age, reach_timeout_s=self.reach_timeout_s, reach_joint_error_rad=error)
        if not self.reach_only and age >= self.reach_timeout_s:
            return self._finish('reach_joint_motion_timeout')
        ...
        if error < .04:
            ...
            lowering_done = self.lowering_m == 0. or self.reach_only or self.lowering_alpha >= 1.
            settled = True
            if not self.reach_only and self.lowering_m == 0.:
    ...
    def describe(self):
        return dict(mode='reach_probe' if self.reach_only else 'first_reach',
                    ...
                    'guarantee, and with zero lowering a missing, rejected or out-of-frame '
                    'detection does not stop the bounded joint motion',
                    ...
                    reach_settle_contract={'linear_norm_m_s': .06, 'angular_norm_rad_s': .12,
                                           ...
                                           'scope': 'zero-lowering reach only; unchanged thresholds '
                                                    'pause the fixed goal instead of stopping the run '
                                                    'on one sample, and a pause is not a stillness claim',
