...
default, and .03 m as one additional candidate, because the audited .025 m
preload with the .20 rad paired lift still slid. Both amplitudes are already
inside the frozen policy's own 0..03 m window, so the deeper one is opened only
by passing it through that untouched constructor, which keeps scaling its own
finite lowering reference and keeps its original descent, hold and guards. Every
post-prefix phase, threshold and bound below is identical either way.
"""
