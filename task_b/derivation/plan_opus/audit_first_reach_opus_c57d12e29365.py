...
def main():
    ...
    with patch('task_b.first_reach.detect_yellow_candidates', detector), \
        ...
        # The strict lowering path keeps the original immediate stop on one
        # instantaneous public twist sample.
        p = ready(lowering_m=.02)
        obs = observation(p)
        obs[0] = .07
        p.act(obs, images)
        checks['lowering_request_keeps_immediate_stop_after_base_motion'] = (
            p.done_reason == 'reach_base_not_stationary')
