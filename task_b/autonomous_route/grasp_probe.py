...
request past the closed stop, built directly here as ``[-.075, +.075]`` m
because the shared ``gripper_targets`` helper only exposes preloads up to
.025 m and that shared helper is deliberately left untouched. A finite position
target makes the unchanged position actuators ask for holding force: it is never
a force injection, and no joint limit, stop, actuator stiffness/damping
(original K=80, D=4, 100 N effort) or helper range is modified, so the actual
finger motion and any actual force remain whatever the original physics
produces. Inputs after entry are the public proprio vector only: no reward,
score, contact sensor, object or simulator state, and no image is read by the
probe.
...
The lowering amplitude of that first reach is a caller parameter: .02 m by
default, and .03 m as one additional candidate, because every recorded run so
far only slid the object and none physically lifted it. Both amplitudes are
already inside the frozen policy's own 0..03 m window, so the deeper one is
opened only by passing it through that untouched constructor, which keeps
scaling its own finite lowering reference and keeps its original descent, hold
and guards. Every post-prefix phase, threshold and bound below is identical
either way.

Relative to the previously audited module this candidate raises the fixed finger
preload to .075 m, widens only q2's measured-reference tether to .18 rad while
the other five axes keep .10 rad, filters a bounded q2 tracking correction
(gain 3, raw cap .12 rad, tau .10 s) onto the same fixed q2 goal, and replaces
the old instantaneous quiet test with an explicit measured stability gate: a
per-tick .12 rad/s arm speed cap plus one complete .5 s measured joint-position
window whose per-axis peak-to-peak span stays within .002 rad. Those numbers
come from offline analysis of recorded runs; the running controller still reads
public observations only, and a nominal spring-force estimate is not a measured
contact force. None of this is a success claim.
"""
...
#: Default body lowering of the frozen first reach; unchanged from the audited run.
DEFAULT_LOWERING_M = .02
#: The two lowering amplitudes this wrapper accepts. Both are already inside the
#: frozen ``FirstReachPolicy`` 0..03 m window, so the deeper candidate is opened
#: purely through that untouched constructor: the frozen policy keeps scaling its
#: own finite lowering reference and keeps its original descent, hold and guards.
#: Nothing about the lowering logic, the states or the leg delta is rewritten
#: here, and the chosen value is passed through unclipped and unrounded.
ALLOWED_LOWERING_M = (.02, .03)
...
# ---------------------------------------------------------------------------
# Constants are fixed within each episode. Candidate selection uses offline
# experiment analysis; the running controller reads only public observations.
# ---------------------------------------------------------------------------
CLOSE_PRELOAD_M = .075          # physical q7=-.075 m, q8=+.075 m
#: Inclusive range this candidate's fixed preload must lie in. The target pair is
#: built directly below because the shared ``gripper_targets`` helper caps its own
#: API at .025 m; that shared helper is NOT modified, and neither are the joint
#: stops or the original K=80/D=4/100 N position actuators. A finite position
#: request past the closed stop asks those unchanged actuators for pressure; it
#: is never a force injection or a physical-limit change.
CLOSE_PRELOAD_RANGE_M = (0., .075)
FINGER_SLEW_M_S = .05           # per finger, from the previous actual command
CLOSE_MAX_S, CLOSE_MIN_ACTIVE_S = 4., 2.4
...
LIFT_MAX_S = 5.
LIFT_RATE_RAD_S = .10           # arm command slew, every axis
LIFT_TETHER_RAD = .10           # measured-reference tether on the five non-q2 axes
LIFT_Q2_TETHER_RAD = .18        # only q2 gets the wider measured-reference tether
LIFT_Q2_FEEDBACK_GAIN = 3.
LIFT_Q2_FEEDBACK_CAP_RAD = .12  # cap on the RAW q2 correction, before the filter
LIFT_Q2_FEEDBACK_TAU_S = .10    # first-order low pass, weight = 1-exp(-dt/tau)
LIFT_ERROR_RAD = .04
LIFT_QDOT_CAP_RAD_S = .12       # per-tick cap on the measured arm speed
#: Complete measured joint-position window required before the arrival timer can
#: run: .5 s of samples, which is 26 samples at 50 Hz counting both endpoints,
#: with a per-axis peak-to-peak span within .002 rad.
LIFT_STILL_WINDOW_S, LIFT_STILL_SPAN_RAD = .5, .002
LIFT_RISE_M = .015
LIFT_ARRIVAL_S = .3
...
CANDIDATE_CONSTANTS = {
    'candidate': 'bounded clamp-and-small-lift candidate selected through offline experiment analysis; '
                 'fixed within each episode and controlled from public observations only',
    'finger_close_target': {'source': 'constructed directly in this module as the finite pair '
                                      f'[-{CLOSE_PRELOAD_M}, +{CLOSE_PRELOAD_M}] m',
                            'not_from_helper': 'task_b.arm_kinematics.gripper_targets is NOT called: its '
                                               'own API allows at most .025 m of preload and that shared '
                                               'helper is left exactly as it is',
                            'allowed_range_m': list(CLOSE_PRELOAD_RANGE_M),
                            ...
                            'note': f'a finite position target {CLOSE_PRELOAD_M} m beyond the closed stop '
                                    ...
                                    'original joint limits, stops and the original K=80/D=4/100 N '
                                    'actuators are untouched and no force is injected, so the actual '
                                    'motion and any actual force stay whatever the original physics gives',
                            'nominal_force_claim': 'the offline nominal spring-force estimate behind this '
                                                   'amplitude is an estimate, not a measured contact force',
    ...
    'close': {'max_s': CLOSE_MAX_S, 'min_active_s': CLOSE_MIN_ACTIVE_S,
              ...
              'nominal_slew_s': f'about 2.2 s to slew the nominal .035 m half-command to the '
                                f'{-CLOSE_PRELOAD_M} m half-target at {FINGER_SLEW_M_S} m/s, so the '
                                f'{CLOSE_MIN_ACTIVE_S} s minimum active close leaves .2 s more, which is '
                                f'also the {WIDTH_WINDOW} consecutive width samples (.2 s at 50 Hz); the '
                                'fingers are never jumped to the target and never re-squeezed'},
    'lift': {'paired_delta_rad': PAIRED_DELTA_RAD, 'joints': ['arm_joint2', 'arm_joint3'],
             ...
             'rate_note': f'the {LIFT_RATE_RAD_S} rad/s arm command slew applies to every axis and is a '
                          f'different quantity from the {FINGER_SLEW_M_S} m/s finger slew',
             'tether_rad': LIFT_TETHER_RAD, 'q2_tether_rad': LIFT_Q2_TETHER_RAD,
             'tether_note': f'only arm_joint2 uses the wider {LIFT_Q2_TETHER_RAD} rad measured-reference '
                            f'tether; the other five axes keep {LIFT_TETHER_RAD} rad. The per-axis rate '
                            'bound on the previous command, the vector tether and the original static '
                            'hard limits are intersected simultaneously, and an empty intersection stops '
                            'while holding the last valid command instead of clipping sequentially',
             'arrival_error_rad': LIFT_ERROR_RAD,
             'arrival_qdot_cap_rad_per_s': LIFT_QDOT_CAP_RAD_S,
             'arrival_position_window_s': LIFT_STILL_WINDOW_S,
             'arrival_position_window_span_rad': LIFT_STILL_SPAN_RAD,
             'arrival_rise_m': LIFT_RISE_M, 'arrival_window_s': LIFT_ARRIVAL_S,
             'arrival_gate': 'combined measured stability gate: every tick the maximum absolute measured '
                             f'arm qdot must stay <= {LIFT_QDOT_CAP_RAD_S} rad/s AND one complete '
                             f'{LIFT_STILL_WINDOW_S} s window of measured arm positions (26 samples at '
                             'the official 50 Hz, both endpoints included; the live count is reported per '
                             'instance) must have a per-axis peak-to-peak span '
                             f'<= {LIFT_STILL_SPAN_RAD} rad, with the goal error, the public FK pinch '
                             'rise and the speed cap eligible on every sample of that window. The window '
                             'is cleared by any ineligible sample, by a pause, by phase entry or exit and '
                             f'by a failure; only once it is ready does the original {LIFT_ARRIVAL_S} s '
                             'continuous arrival hold start. No instantaneous velocity test remains, and '
                             'this window is used for LIFT arrival only',
             'progress_window_s': LIFT_PROGRESS_WINDOW_S, 'progress_rad': LIFT_PROGRESS_RAD,
             ...
             'q2_raw_correction_cap_rad': LIFT_Q2_FEEDBACK_CAP_RAD,
             'q2_correction_filter_tau_s': LIFT_Q2_FEEDBACK_TAU_S,
             'feedback_basis': 'only arm_joint2 is corrected: the raw correction is '
                               f'clip({LIFT_Q2_FEEDBACK_GAIN}*(fixed q2 goal - measured q2), '
                               f'+/-{LIFT_Q2_FEEDBACK_CAP_RAD} rad) and a first-order low pass with '
                               f'tau={LIFT_Q2_FEEDBACK_TAU_S} s (weight 1-exp(-dt/tau)) is added to the '
                               'same fixed q2 goal before the bounded intersection. The other five axes '
                               'get no correction. The filter history is zeroed exactly once on entering '
                               'LIFT, is frozen together with the commands during a pause and updates '
                               'once on the first resumed active tick; LIFT->OBSERVE does not reset it, '
                               'and OBSERVE keeps tracking the SAME fixed goal under the same rate, '
                               'tether and hard bounds. The fixed actual joint goal, the arrival '
                               'thresholds and the original actuator settings stay unchanged'},
    'observe': {'window_s': OBSERVE_S,
                'note': 'the original 2 s same-goal closure, hold and public motion checks are kept; the '
                        'new measured position window gates LIFT arrival only'},
    ...
    'deadline_note': 'every phase deadline and the total probe budget count pause time; nothing here '
                     'silently extends them',
    'required_first_reach': {'reach_only': False,
                             'default_lowering_m': DEFAULT_LOWERING_M,
                             'allowed_lowering_m': list(ALLOWED_LOWERING_M),
                             'lowering_note': 'the wrapper accepts either amplitude and hands it to the '
                                              'frozen FirstReachPolicy unchanged, which already permits '
                                              '0..03 m and scales its own finite lowering reference; the '
                                              'per-instance value is reported as first_reach_lowering_m, '
                                              'no lowering state or leg delta is added or edited here, '
                                              'and the deeper amplitude was picked by offline analysis of '
                                              'the recorded original mesh and pose, not at runtime',
...
STOP_REASONS = {
    ...
    'grasp_probe_lift_not_reached': 'the arrival conditions (goal error, public FK pinch rise, the '
                                    f'{LIFT_QDOT_CAP_RAD_S} rad/s measured speed cap, one complete '
                                    f'{LIFT_STILL_WINDOW_S} s measured position window within '
                                    f'{LIFT_STILL_SPAN_RAD} rad and then {LIFT_ARRIVAL_S} s of continuous '
                                    f'arrival) were not met inside {LIFT_MAX_S} s',
    'grasp_probe_rate_tether_intersection_empty': 'the per-axis rate bound on the previous command, the '
                                                  f'measured tether ({LIFT_TETHER_RAD} rad, '
                                                  f'{LIFT_Q2_TETHER_RAD} rad on q2) and the original '
                                                  'static hard limits had no common command, so the last '
                                                  'valid command is held instead of clipped sequentially',
...
class GraspProbePolicy:
    ...
    ``reach_lowering_hold_complete`` is intercepted. ``lowering_m`` must be one of
    :data:`ALLOWED_LOWERING_M` and is forwarded verbatim, so at the same amplitude
    the prefix matches the frozen policy tick for tick.
    """
    ...
    def __init__(self, schema, observation_joint_names, defaults, dt=.02, *,
                 ...
                 lowering_m=DEFAULT_LOWERING_M):
        if reach_only:
            raise ValueError('The grasp probe needs the real visual first reach; reach_only must be False')
        allowed = ', '.join(repr(value) for value in ALLOWED_LOWERING_M)
        if np.ndim(lowering_m) != 0:
            raise ValueError('lowering_m must be one finite scalar, one of ' + allowed + ' m')
        try:
            lowering = float(lowering_m)
        except (TypeError, ValueError):
            raise ValueError('lowering_m must be one finite scalar, one of ' + allowed + ' m')
        if not np.isfinite(lowering) or not any(abs(lowering-value) <= 1e-9
                                                for value in ALLOWED_LOWERING_M):
            # Only these audited amplitudes are opened, and an accepted value is
            # forwarded exactly as given: never clipped or rounded onto one of them.
            raise ValueError('The grasp probe is defined only for a lowered reach of ' + allowed
                             + ' m (default ' + repr(DEFAULT_LOWERING_M) + ' m); got '
                             + repr(lowering_m))
        self._inner = FirstReachPolicy(schema, observation_joint_names, defaults, dt=dt,
                                       ...
                                       standoff=standoff, turn_gain=turn_gain, lowering_m=lowering)
        ...
        # Built directly here, NOT through gripper_targets: that shared helper
        # allows at most .025 m of preload and is deliberately left untouched.
        # This is one finite position request past the closed stop, mapped by the
        # existing arm_targets_to_action affine mapping (position/default offset/
        # scale, official clip None); it never injects force and never edits a
        # joint stop or the original K=80/D=4/100 N actuators.
        self.finger_target = np.array([-CLOSE_PRELOAD_M, CLOSE_PRELOAD_M], dtype=float)
        if not (np.isfinite(self.finger_target).all()
                and CLOSE_PRELOAD_RANGE_M[0] < CLOSE_PRELOAD_M <= CLOSE_PRELOAD_RANGE_M[1]
                and np.allclose(self.finger_target, [-CLOSE_PRELOAD_M, CLOSE_PRELOAD_M], atol=1e-12)):
            raise ValueError('The fixed physical finger target must be one finite [-preload, +preload] '
                             'pair with the documented candidate preload in ('
                             + repr(CLOSE_PRELOAD_RANGE_M[0]) + ', ' + repr(CLOSE_PRELOAD_RANGE_M[1])
                             + '] m; got ' + repr(CLOSE_PRELOAD_M))
        # Per-axis measured tether: only q2 is widened, the other five keep .10.
        self.lift_tether = self._tether_vector()
        # .5 s of measured arm positions, both endpoints included: 26 samples at
        # the official 50 Hz. Only LIFT arrival consumes this window.
        self.still_samples = int(np.ceil(LIFT_STILL_WINDOW_S/self.dt))+1
        self.still_window = deque(maxlen=self.still_samples)
        self.still_span, self.still_ready = None, False
        ...
        self.q2_raw_correction_rad, self.q2_position_feedback_rad = 0., 0.
        self.q2_filter_resets = 0
    ...
    def _stop(self, reason):
        ...
        self.state = 'STOPPED'
        self._hold = True
        self._clear_still_window()
        action = self._action()
        self._record()
        return action

    def _clear_still_window(self):
        """Drop the measured position window; a failure never keeps it ready."""
    ...
    def _pause(self, forced):
        ...
        lowered reference and the wheel anchor stay exactly as they are. The q2
        correction filter history is frozen with them: no tick of a pause updates
        it, and the first resumed active tick updates it exactly once. The width,
        measured position, arrival and observation windows restart afterwards;
        phase deadlines, the body budgets and the total probe budget never do.
    ...
    def _close(self, q):
        ...
        non-empty measured width window. Nominally the fingers need about 2.2 s
        to slew from .035 m to the -.075 m half-target at .05 m/s, so the 2.4 s
        minimum active close leaves .2 s more, exactly the .2 s the ten-sample
        width window needs. The fingers never jump to the target and the phase
        deadline is not extended for a slow slew. That window only permits the
        bounded lift below. It is NOT a claim of contact or grasp: no contact
        sensor, score or object state is read, and the fingers are never
        re-squeezed or retried.
        """
    ...
    def _enter_lift(self, q):
        ...
        # Phase entry starts the measured position window empty and zeroes the q2
        # correction filter exactly once. LIFT -> OBSERVE does NOT zero it again.
        self._clear_still_window()
        self.q2_raw_correction_rad, self.q2_position_feedback_rad = 0., 0.
        self.q2_filter_resets += 1
        return self._action()

    def _q2_correction(self, q):
        """Bounded, low-passed q2-only correction toward the same fixed goal.

        The raw term is the capped proportional error on public measured q2; the
        first-order filter with ``LIFT_Q2_FEEDBACK_TAU_S`` smooths it. Called only
        on active LIFT/OBSERVE ticks, so a pause freezes this history with the
        commands and the first resumed active tick updates it exactly once.
        """
        raw = float(np.clip(LIFT_Q2_FEEDBACK_GAIN*(self.lift_goal[1]-q[1]),
                            -LIFT_Q2_FEEDBACK_CAP_RAD, LIFT_Q2_FEEDBACK_CAP_RAD))
        weight = float(1.-np.exp(-self.dt/LIFT_Q2_FEEDBACK_TAU_S))
        self.q2_raw_correction_rad = raw
        self.q2_position_feedback_rad = float(self.q2_position_feedback_rad
                                              + weight*(raw-self.q2_position_feedback_rad))
        return self.q2_position_feedback_rad

    def _tether_vector(self):
        """Per-axis measured-reference tether: only q2 is widened to .18 rad.

        The other five axes keep .10 rad. This is a bound on the command relative
        to public measured q; it is not an actuator, limit or stiffness change.
        """
        tether = np.full(6, LIFT_TETHER_RAD)
        tether[1] = LIFT_Q2_TETHER_RAD
        return tether

    def _track_lift_goal(self, q):
        """Move the six commands toward the latched goal under all three bounds."""
        # Satisfy the previous-command rate bound, the per-axis measured tether
        # and the original static hard limits simultaneously. Sequential clipping
        # can jump farther than the rate limit after a sudden measured q change.
        rate = LIFT_RATE_RAD_S*self.dt
        self.lift_tether = tether = self._tether_vector()
        low = np.maximum(np.maximum(self.arm_command[:6]-rate, q[:6]-tether),
                         self.static_lower)
        high = np.minimum(np.minimum(self.arm_command[:6]+rate, q[:6]+tether),
                          self.static_upper)
        if np.any(low > high):
            # Hold the last valid command; nothing is clipped onto one bound.
            return self._stop('grasp_probe_rate_tether_intersection_empty')
        command_goal = self.lift_goal.copy()
        # Only q2 is corrected, and only on the command: the latched goal itself
        # never moves, and the other five axes track it with no correction.
        command_goal[1] += self._q2_correction(q)
        bounded = np.clip(command_goal, low, high)
        if not np.array_equal(bounded, command_goal):
            self.lift_clamped_calls += 1
        self.arm_command[:6] = bounded
    ...
    def _still_window(self, q, eligible):
        """Maintain the complete measured joint-position window for LIFT arrival.

        An ineligible sample clears it, so the window can only become ready when
        every one of its ``still_samples`` measurements satisfied the goal error,
        the public FK rise and the hard speed cap. Readiness additionally needs a
        full window whose per-axis measured peak-to-peak span is within
        ``LIFT_STILL_SPAN_RAD``. Pauses, phase entry/exit and failures clear it.
        """
        if not eligible:
            self._clear_still_window()
            return False
        self.still_window.append(np.asarray(q[:6], dtype=float).copy())
        if len(self.still_window) < self.still_samples:
            self.still_span, self.still_ready = None, False
            return False
        self.still_span = float(np.max(np.ptp(np.asarray(self.still_window), axis=0)))
        self.still_ready = bool(self.still_span <= LIFT_STILL_SPAN_RAD)
        return self.still_ready
    ...
    def _lift(self, q, arm_qdot, up):
        ...
        # Combined measured stability gate: the per-tick speed cap and one
        # complete measured position window replace the old instantaneous test.
        eligible = bool(self.lift_error < LIFT_ERROR_RAD and self.lift_qdot <= LIFT_QDOT_CAP_RAD_S
                        and self.pinch_rise >= LIFT_RISE_M)
        if self._still_window(q, eligible):
            ...
            if (self.calls-self.lift_arrival_start+1)*self.dt >= LIFT_ARRIVAL_S:
                ...
                # Phase exit clears the position window; the q2 filter history is
                # deliberately NOT reset, so compensation continues in OBSERVE.
                self._clear_still_window()
                return self._action()
        else:
            self.lift_arrival_start = None
    ...
    def _observe(self, q, arm_qdot, up):
        ...
        ``_track_lift_goal`` keeps running against the SAME fixed latched goal
        under the same rate bound, per-axis measured tether and static hard
        limits, and the q2 correction filter keeps its history across the phase
        change: compensation is not frozen merely because the phase changed. The
        original 2 s closure, hold and public motion checks are unchanged, and
        the new measured position window is not used here. The experiment ends
        here: no transport, opening, regrasp or further lowering follows, and no
        image, object motion or score is inspected.
        """
    ...
    def _record(self):
        ...
        debug.update(
            ...
            probe_q2_raw_correction_rad=self.q2_raw_correction_rad,
            probe_q2_filtered_correction_rad=self.q2_position_feedback_rad,
            probe_q2_position_feedback_rad=self.q2_position_feedback_rad,
            probe_q2_feedback_basis={'axis': 'arm_joint2', 'gain': LIFT_Q2_FEEDBACK_GAIN,
                                     'raw_cap_rad': LIFT_Q2_FEEDBACK_CAP_RAD,
                                     'filter_tau_s': LIFT_Q2_FEEDBACK_TAU_S,
                                     'filter_weight': float(1.-np.exp(-self.dt/LIFT_Q2_FEEDBACK_TAU_S)),
                                     'history_resets': self.q2_filter_resets,
                                     'note': 'probe_q2_position_feedback_rad IS the filtered correction '
                                             'actually added to the same fixed q2 goal; the other five '
                                             'axes get no correction, a pause freezes this history and '
                                             'LIFT->OBSERVE does not reset it'},
            ...
            probe_finger_target_source=f'constructed directly as [-{CLOSE_PRELOAD_M}, +{CLOSE_PRELOAD_M}] m; '
                                       'the shared gripper_targets helper (.025 m API limit) is not '
            ...
            probe_lift_tether_rad=self.lift_tether.tolist(),
            probe_lift_tether_basis=f'arm_joint2 {LIFT_Q2_TETHER_RAD} rad, the other five axes '
                                    f'{LIFT_TETHER_RAD} rad; the {LIFT_RATE_RAD_S} rad/s command slew '
                                    'applies to every axis and is intersected with this tether and the '
                                    'original static hard limits at once',
            ...
            probe_lift_qdot_cap_rad_s=LIFT_QDOT_CAP_RAD_S,
            probe_lift_position_window_samples=len(self.still_window),
            probe_lift_position_window_required_samples=self.still_samples,
            probe_lift_position_window_s=max(0., (len(self.still_window)-1)*self.dt),
            probe_lift_position_window_required_s=LIFT_STILL_WINDOW_S,
            probe_lift_position_window_span_rad=self.still_span,
            probe_lift_position_window_span_limit_rad=LIFT_STILL_SPAN_RAD,
            probe_lift_position_window_ready=bool(self.still_ready),
            probe_lift_arrival_gate=f'LIFT arrival only: every tick max measured arm |qdot| <= '
                                    f'{LIFT_QDOT_CAP_RAD_S} rad/s, one complete {LIFT_STILL_WINDOW_S} s '
                                    f'({self.still_samples} samples, both endpoints) measured position '
                                    f'window with per-axis span <= {LIFT_STILL_SPAN_RAD} rad and the goal '
                                    'error/FK rise eligible on every sample, then the unchanged '
                                    f'{LIFT_ARRIVAL_S} s continuous arrival hold. No instantaneous '
                                    'velocity test is used, and OBSERVE keeps its original 2 s checks',
            ...
            probe_first_reach_lowering_m=self._inner.lowering_m,
            probe_first_reach_lowering_basis='the amplitude this instance actually gave the frozen '
                                             ...
                                             'own scaled lowering reference; the static candidate '
                                             f'metadata lists the default {DEFAULT_LOWERING_M} m and the '
                                             f'allowed {list(ALLOWED_LOWERING_M)} m',
    ...
    def describe(self):
        return dict(mode='grasp_probe',
                    ...
                    lowering_contract='this instance handed the frozen policy the lowering amplitude '
                                      'reported in first_reach_lowering_m, one of the allowed '
                                      f'{list(ALLOWED_LOWERING_M)} m with {DEFAULT_LOWERING_M} m as the '
                                      'default. The frozen policy already permits 0..03 m and scales its '
                                      'own finite lowering reference, so no lowering state, descent, hold '
                                      'or leg delta is added or edited here, and the prefix stays '
                                      'identical to the frozen policy run at the same amplitude. The '
                                      'deeper amplitude was chosen from offline analysis of the recorded '
                                      'original mesh and pose; the running controller reads only public '
                                      'observations',
                    phases={'PROBE_CLOSE': 'hold the six achieved joint commands, slew both fingers to '
                                           ...
                                           f'[{WIDTH_MIN_M},{WIDTH_MAX_M}] m. That target pair is built '
                                           'directly in this module, not through gripper_targets, whose '
                                           'own API stops at .025 m and which is left untouched. A finite '
                                           'target beyond the closed stop requests holding force from the '
                                           'unchanged position actuators; no force is injected and the '
                                           'joint limits, stops and original K=80/D=4/100 N actuators are '
                                           'not modified. Nominally the .035 m half-command needs 2.2 s '
                                           f'to reach {-CLOSE_PRELOAD_M} m, so the {CLOSE_MIN_ACTIVE_S} s '
                                           'minimum leaves .2 s more, with no jump and no re-squeeze. '
                                           'This permits the bounded lift only; it is not a claim of '
                                           'contact or grasp, and there is no retry',
                            'PROBE_LIFT': 'latch measured arm q once, apply the fixed paired goal '
                                          ...
                                          f'under a {LIFT_RATE_RAD_S} rad/s per-axis command slew and a '
                                          f'per-axis measured tether of {LIFT_Q2_TETHER_RAD} rad on q2 '
                                          f'and {LIFT_TETHER_RAD} rad on the other five, intersected with '
                                          'the original static hard limits simultaneously (an empty '
                                          'intersection stops and holds the last valid command) while the '
                                          'closed finger target is retained. Only q2 receives a bounded '
                                          f'correction: raw clip(+/-{LIFT_Q2_FEEDBACK_CAP_RAD} rad) of '
                                          f'{LIFT_Q2_FEEDBACK_GAIN}x the public measured q2 error, '
                                          f'low-passed with tau={LIFT_Q2_FEEDBACK_TAU_S} s and added to '
                                          'the same fixed goal. Arrival needs the measured goal error '
                                          f'<{LIFT_ERROR_RAD} rad, public FK pinch rise '
                                          f'>={LIFT_RISE_M} m, every tick max measured arm |qdot| '
                                          f'<={LIFT_QDOT_CAP_RAD_S} rad/s and one complete '
                                          f'{LIFT_STILL_WINDOW_S} s measured position window '
                                          f'({self.still_samples} samples at {1./self.dt:.0f} Hz, both '
                                          'endpoints included) with per-axis span '
                                          f'<={LIFT_STILL_SPAN_RAD} rad, then {LIFT_ARRIVAL_S} s of '
                                          'continuous arrival. No Cartesian IK or grasp-point shift takes '
                                          'part',
                            'PROBE_OBSERVE': 'hold the same measured goal, closed target, lowered leg '
                                             'reference, wheel anchor and motion bounds for '
                                             f'{OBSERVE_S} s under the same rate, tether and hard bounds, '
                                             'with the q2 correction filter continuing across the phase '
                                             'change and the original 2 s checks unchanged, then end. The '
                                             'experiment stops there: no transport, opening, regrasp or '
                                             'further lowering'},
                    ...
                    probe_state={'phase': self.phase, 'phase_order': list(PROBE_PHASES),
                                 ...
                                 'finger_preload_m': CLOSE_PRELOAD_M,
                                 ...
                                 'lift_tether_rad': self.lift_tether.tolist(),
                                 'lift_max_q_error_rad': self.lift_error,
                                 'lift_max_qdot_rad_s': self.lift_qdot,
                                 'lift_qdot_cap_rad_s': LIFT_QDOT_CAP_RAD_S,
                                 'lift_position_window_samples': len(self.still_window),
                                 'lift_position_window_required_samples': self.still_samples,
                                 'lift_position_window_span_rad': self.still_span,
                                 'lift_position_window_ready': bool(self.still_ready),
                                 'q2_raw_correction_rad': self.q2_raw_correction_rad,
                                 'q2_filtered_correction_rad': self.q2_position_feedback_rad,
                                 'q2_correction_filter_tau_s': LIFT_Q2_FEEDBACK_TAU_S,
                                 'q2_filter_history_resets': self.q2_filter_resets,
                    ...
                    verification='real validation is external: a fresh original-environment run plus an '
                                 'independent audit of the same object\'s recorded pose, quaternion and '
                                 'mesh lower surface, which needs at least 10 mm of mesh-bottom clearance '
                                 'together with at least 15 mm of object-root rise and about 1 s of the '
                                 'object following the gripper relatively. Jaw width, a proximity score '
                                 'and the policy phase certify nothing. This policy cannot and does not '
                                 'certify a grasp, and no transport is implemented',
