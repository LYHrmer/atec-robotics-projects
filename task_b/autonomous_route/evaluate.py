...
parser.add_argument("--mode", choices=("hold", "forward", "turn", "crouch", "visual_approach", "first_reach", "multi_reach", "grasp_probe", "contact_grasp", "payload_motion", "carry_probe", "first_delivery", "official_locomotion", "reach_probe"), default="hold")
parser.add_argument("--reach_attempts", type=int, default=2, help="Bounded number of visual reach attempts in multi_reach; an attempt is not an awarded point.")
...
parser.add_argument("--payload_open_on", choices=("observation_complete", "close_complete"), default="observation_complete", help="Where payload_motion takes over from the contact-grasp child; close_complete is the measured loaded-lift entry, never a reclassified failure.")
parser.add_argument("--carry_probe_forward", type=float, default=.20, help="carry_probe: bounded loaded straight-run target in metres. A NEW probe initial value, not a measured capability.")
parser.add_argument("--carry_probe_speed", type=float, default=.03, help="carry_probe: target planar speed in m/s. A NEW probe initial value.")
parser.add_argument("--carry_probe_yaw", type=float, default=.175, help="carry_probe: bounded yaw probe target in rad, each direction. A NEW probe initial value.")
parser.add_argument("--low_place", action="store_true",
                    help="first_delivery: extend forward and lower the load before opening, so a restitution-1 bounce cannot leave the barrel. CPU-verified geometry; the IK is solved from the measured pose so it needs a real run.")
parser.add_argument("--leg_twist_rad", type=float, default=0.,
                    help="first_delivery: hip-twist yaw in radians of front/rear abduction. 0 disables it (default). The leg actuators are far stronger than the wheels.")
parser.add_argument("--leg_twist_sign", type=float, default=1.,
                    help="first_delivery: which way the hip twist yaws the body. The mirrored legs make this uncertain from the asset alone, so the first run measures it.")
parser.add_argument("--dock_bearing_tol_rad", type=float, default=None,
                    help="first_delivery: how far off the barrel bearing the dock may be. Defaults to what the arm can absorb; tighten it once the chassis can actually turn.")
parser.add_argument("--drive_wheel_speed_max_rad_s", type=float, default=None,
                    help="Override the drive layer's wheel speed cap (physical rad/s). The wheel drive has stiffness 0 / damping 1.0, so its torque is 1.0*(commanded-actual) and the 20 N*m effort limit is only reached at a 20 rad/s error; the default cap may be what limits a turn.")
parser.add_argument("--drive_differential_max_rad_s", type=float, default=None,
                    help="Override the drive layer's differential cap (physical rad/s).")
parser.add_argument("--locomotion_command", type=float, nargs=3, default=[0., 0., 0.],
                    metavar=("VX", "VY", "WZ"), help="official_locomotion: velocity command fed to the provided checkpoint.")
parser.add_argument("--carry_probe_total", type=float, default=60., help="carry_probe: whole-probe wall budget in seconds after the handoff.")
...
if args.score_hold_steps < 0:
    parser.error("--score_hold_steps must be >= 0")
...
if args.mode in ("first_reach", "multi_reach", "grasp_probe", "contact_grasp", "payload_motion", "carry_probe", "first_delivery", "reach_probe") and not 0 <= args.reach_lowering <= .03:
    parser.error("--reach_lowering must be in [0,.03] m")
if args.brake_wheel_hold and args.mode not in ("first_reach", "multi_reach", "grasp_probe", "contact_grasp", "payload_motion", "carry_probe", "first_delivery"):
    parser.error("--brake_wheel_hold requires first_reach, multi_reach, grasp_probe, contact_grasp, payload_motion, carry_probe or first_delivery")
if args.mode in ("first_reach", "multi_reach", "grasp_probe", "contact_grasp", "payload_motion", "carry_probe", "first_delivery") and args.reach_lowering > 0 and not (
        args.brake_wheel_hold and args.stance_hold and args.stance_profile == "compact"):
    parser.error("reach lowering requires --brake_wheel_hold --stance_hold --stance_profile compact")
if args.mode == "multi_reach" and (not 1 <= args.reach_attempts <= 3 or not (
        args.brake_wheel_hold and args.stance_hold and args.stance_profile == "compact")):
    parser.error("multi_reach requires 1..3 reach attempts, brake wheel hold and compact stance hold")
if args.mode in ("grasp_probe", "contact_grasp", "payload_motion", "carry_probe", "first_delivery") and not (args.reach_lowering > 0 and args.brake_wheel_hold
                                      and args.stance_hold and args.stance_profile == "compact"):
    parser.error("grasp_probe/contact_grasp/payload_motion/carry_probe/first_delivery requires positive fixed lowering, brake wheel hold and compact stance hold")
...
def sample_state(task, illegal: dict | None) -> dict:
    ...
    return {
        ...
        "finger_contact_forces_w": (task.scene.sensors["contact_sensor"].data.net_forces_w[
            0, [task.scene.sensors["contact_sensor"].body_names.index(name)
                for name in ("arm_link7", "arm_link8")]].detach().cpu().numpy().copy()
            if "contact_sensor" in task.scene.sensors else np.zeros((2, 3))),
        "gripper_xyz": robot.data.body_pos_w[0, robot.data.body_names.index("gripper_base")].detach().cpu().numpy().copy(),
        "gripper_quat": robot.data.body_quat_w[0, robot.data.body_names.index("gripper_base")].detach().cpu().numpy().copy(),
        "object_xyz": np.stack([task.scene[name].data.root_pos_w[0].detach().cpu().numpy().copy() for name in OBJECT_NAMES]),
        "object_quat": np.stack([task.scene[name].data.root_quat_w[0].detach().cpu().numpy().copy() for name in OBJECT_NAMES]),
    }
...
def main() -> None:
    ...
    telemetry = {key: [] for key in ("step", "sim_seconds", "alpha", "action", "requested_action", "proprio", "q", "qdot", "base_xyz",
                                     ...
                                     "termination", "reward_terms", "gripper_xyz", "gripper_quat", "object_xyz", "object_quat",
                                     "finger_contact_forces_w")}
    try:
        ...
        visual_mode = args.mode in ("visual_approach", "first_reach", "multi_reach", "grasp_probe", "contact_grasp", "payload_motion", "carry_probe", "first_delivery", "reach_probe")
        if args.mode in ("first_reach", "multi_reach", "grasp_probe", "contact_grasp", "payload_motion", "carry_probe", "first_delivery", "official_locomotion", "reach_probe"):
            from task_b.first_reach import FirstReachPolicy
            policy_class, extra = FirstReachPolicy, {}
            if args.mode == "multi_reach":
                from task_b.multi_reach import MultiReachPolicy
                policy_class, extra = MultiReachPolicy, {"max_attempts": args.reach_attempts}
            elif args.mode == "grasp_probe":
                from task_b.grasp_probe import GraspProbePolicy
                policy_class = GraspProbePolicy
            elif args.mode == "contact_grasp":
                from task_b.contact_grasp import ContactGraspPolicy
                policy_class = ContactGraspPolicy
            elif args.mode == "payload_motion":
                from task_b.payload_motion import PayloadMotionPolicy
                policy_class = PayloadMotionPolicy
                extra = {"open_on": args.payload_open_on}
            elif args.mode == "official_locomotion":
                from task_b.official_locomotion import OfficialLocomotionPolicy
                policy_class = OfficialLocomotionPolicy
                extra = {"command": tuple(args.locomotion_command)}
            elif args.mode in ("carry_probe", "first_delivery"):
                from task_b.delivery import PayloadDeliveryPolicy
                policy_class = PayloadDeliveryPolicy
                extra = {"mode": args.mode, "open_on": "close_complete",
                         "wheel_action_gain": args.wheel_action_gain,
                         "probe_forward_m": args.carry_probe_forward,
                         "probe_speed_m_s": args.carry_probe_speed,
                         "probe_yaw_rad": args.carry_probe_yaw,
                         "probe_total_s": args.carry_probe_total,
                         "dock_bearing_tol_rad": args.dock_bearing_tol_rad,
                         "low_place": args.low_place,
                         "leg_twist_rad": args.leg_twist_rad,
                         "leg_twist_sign": args.leg_twist_sign,
                         "drive_wheel_speed_max_rad_s": args.drive_wheel_speed_max_rad_s,
                         "drive_differential_max_rad_s": args.drive_differential_max_rad_s}
            if args.mode == "official_locomotion":
                policy = policy_class(schema, observed_names, defaults, dt=dt, **extra)
            else:
                policy = policy_class(schema, observed_names, defaults, dt=dt,
                                      ...
                                      turn_gain=args.reach_turn_gain, lowering_m=args.reach_lowering, **extra)
        ...
        # Read-only physics diagnostics. These values never enter policy.act.
        material_diagnostics = {}
        for name in (ROBOT, *OBJECT_NAMES):
            try:
                view = task.scene[name].root_physx_view
                material_diagnostics[name] = {
                    "material_properties": jsonable(view.get_material_properties()[0]),
                    "masses": jsonable(view.get_masses()[0]),
                }
                if name == ROBOT:
                    material_diagnostics[name]["joint_names"] = list(robot.data.joint_names)
                    material_diagnostics[name]["drive_stiffness"] = jsonable(view.get_dof_stiffnesses()[0])
                    material_diagnostics[name]["drive_damping"] = jsonable(view.get_dof_dampings()[0])
                    material_diagnostics[name]["drive_max_force"] = jsonable(view.get_dof_max_forces()[0])
            except Exception as error:
                material_diagnostics[name] = {"unavailable": str(error)}
        ...
        metadata = {
            ...
            "physics_diagnostics": {
                "source": "original live root_physx_view read-only getters after reset; no policy input",
                "material_columns": ["static_friction", "dynamic_friction", "restitution"],
                "assets": material_diagnostics,
                "finger_contact_body_names": ["arm_link7", "arm_link8"],
                "finger_contact_forces": "existing sensor net_forces_w contains summed NORMAL contact forces only, world frame, pre-step; friction is not included and these are not total support forces",
            },
            ...
            "observations": {"proprio_dim": proprio_dim,
                             ...
                             "policy_inputs": ("proprio + head/ee RGB-D" if args.vision_head or args.mode in ("first_reach", "multi_reach", "grasp_probe", "contact_grasp", "payload_motion", "carry_probe", "first_delivery", "reach_probe") else "proprio + ee RGB-D")
            ...
            "brake_wheel_hold_integration": {
                "active_states": None if hasattr(policy, "wheel_hold_requested") else brake_states,
                "activation_source": ("policy.wheel_hold_requested" if hasattr(policy, "wheel_hold_requested")
                                      else "policy.state in active_states"),
                "fallback_active_states": brake_states,
        ...
        for step in range(args.max_steps):
            ...
            if args.mode in ("first_reach", "multi_reach", "grasp_probe", "contact_grasp", "payload_motion", "carry_probe", "first_delivery", "reach_probe"):
            ...
            if brake_holder:
                if getattr(policy, "wheel_hold_requested", policy.state in brake_states):
            ...
            telemetry["gripper_quat"].append(state["gripper_quat"])
            telemetry["object_xyz"].append(state["object_xyz"])
            telemetry["object_quat"].append(state["object_quat"])
            telemetry["finger_contact_forces_w"].append(state["finger_contact_forces_w"])
