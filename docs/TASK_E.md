...
[仓库首页](../README.md) · [Task A](../task_a/README.md) · [跨任务学习路线](ATEC_PROJECTS.md)
...
另已完成 [模仿学习实验](TASK_E_IMITATION.md)：从教师演示学习六轴关节动作修正，在独立 seed 1 获得 **18/18 分、52.84 秒**；同场景原规则和纯 DLS 分别为 62.96 / 63.16 秒。它保留视觉、规划与抓放规则，当前每组仅一次评测。本页继续介绍原几何方案；学习版本的数据、权重、对照和运行命令集中在实验说明中。

[![播放三物体抓放视频](../media/fast_preview.jpg)](../media/task_e_fast_seed42_color.mp4)

[彩色演示视频](../media/task_e_fast_seed42_color.mp4) · [原始录像](../media/task_e_fast_seed42_original.mp4) · [视频与代码包下载](https://github.com/LYHrmer/atec-robotics-projects/releases/tag/fast-smooth-18)

## 从哪里开始读

第一次接触项目，建议从 [项目导读](LEARNING_GUIDE.md) 开始。它从视频中的具体动作讲起，再把感知、运动学和状态机连到一起。

| 想弄清楚的事情 | 对应资料 |
| --- | --- |
| 整个任务怎么完成，各模块怎样配合 | [项目导读](LEARNING_GUIDE.md) |
| 像素怎样变成夹持点，IK 又在求什么 | [算法原理与公式](ALGORITHM.md) |
| 一次 `predicts` 调用会经过哪些代码 | [代码导读](CODE_WALKTHROUGH.md) |
| 不开仿真，先自己算一遍关键步骤 | [五个动手练习](HANDS_ON.md) |
| 为什么能省下 8.06 秒，动作指标怎样比较 | [优化记录](OPTIMIZATION.md) |
| 模仿学习究竟学了什么，怎样与规则和纯 DLS 比较 | [模仿学习实验](TASK_E_IMITATION.md) |
| 怎样讲项目，以及如何检查自己是否读懂 | [面试讲解](INTERVIEW.md) · [自测题](SELF_CHECK.md) |
...
视频对应 v3 的 seed 42 源版本，打包时只规范化了导入路径；独立包随后复现相同步数和分数。详细对应关系见 [录像源码记录](../results/fast_seed42_source.json)，旧版代码与视频保留在 [baseline-18](https://github.com/LYHrmer/atec-robotics-projects/releases/tag/baseline-18)。
...
原几何方案使用固定外部 RGB-D；视频中的腕部相机用于观察和诊断。机器人尺寸、标定和固定桌篮配置来自公开任务定义，随机物体位置由图像估计。运行策略不读取真实物体位姿，原方案也不需要训练权重 `policy.pt`。

## 本地复现

以下命令均从仓库根目录执行，统一启动器 `run.sh task-e` 转入现有 Task E 评测脚本；策略入口为 `solution.py`。
...
bash run.sh task-e --check
bash run.sh task-e --solution solution.py --headless --seed 42 \
...
官方入口是 [solution.py](../solution.py) 中的 `AlgSolution.predicts(obs, current_score)`，返回八维 `action` 和布尔 `giveup`。运行时需要根目录的七个策略 Python 文件；继承关系、调用顺序和动作尺度见 [代码导读](CODE_WALKTHROUGH.md)。
