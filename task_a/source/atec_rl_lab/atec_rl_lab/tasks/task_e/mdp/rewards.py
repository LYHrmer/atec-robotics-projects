...
class ObjectsInBasket(ManagerTermBase):
	...
	def __call__(
		...
		# [DIAG] 打印本步首次进篮的物体身份(仅 env 0），用于区分完成的是 obj1/obj2/obj3
		if bool(newly_inside[0].any()):
			ids = [self._object_names[i] for i in range(newly_inside.shape[1]) if bool(newly_inside[0, i])]
			print(f"[DIAG] IN_BASKET env0: {ids}", flush=True)
...
class GraspedObjectsByEE(ManagerTermBase):
	...
	def __call__(
		...
		# [DIAG] 打印本步首次抓起的物体身份(仅 env 0），用于区分完成的是 obj1/obj2/obj3
		if bool(newly_grasped[0].any()):
			ids = [self._object_names[i] for i in range(newly_grasped.shape[1]) if bool(newly_grasped[0, i])]
			print(f"[DIAG] GRASPED env0: {ids}", flush=True)
